...
contract ItemFactory is ERC1155SupplyCC, AccessControl {
    ...
    // Added by prevent overrite function issue
    ...
    function claim(address claimer, uint256 entropy) external {
        ...
        // handle MILK or ITEMS
            ...
            LootData memory lootData = LootData(min, max, ids);

            uint256 petTokenId = 0;
            uint256 lastUpdateForID = _lastUpdate[petTokenId];
            if (lastUpdateForID == 0)
                lastUpdateForID = block.timestamp + 60 * 60 * 24;

            uint256 multiplier;
            // time
            unchecked {
                multiplier =
                    (block.timestamp - lastUpdateForID) /
                    (60 * 60 * 24);
            }
            require(multiplier > 0, "Can claim only once per day");

            // do some bit shifting magic to create random min max
            uint256 rewardAmount = lootData.min +
                ((randomNum(entropy)) % (lootData.max - lootData.min + 1));
    ...
    function randomNum(uint256 entropy) internal returns (uint256) {
        return
            uint256(
                keccak256(
